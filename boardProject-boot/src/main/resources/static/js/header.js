...
// async : 비동기가 수행되는 함수 정의 부분 앞에 붙여서 사용하는 키워드 (비동기 요청을 수행할 것 이다)
...
async function getServiceKey() {
    try {
        const response = await fetch("/getServiceKey"); // 첫 번째 then의 response(Promise) 객체 리턴
...
async function fetchData() {
    ...
    console.log("test : " + serviceKey);
    
    const url = "http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst";

    // URLSearchParams : URL의 쿼리문자열을 쉽게 다룰 수 있게 해주는 JS 내장 객체
    // 단, 사용 시 decode 서비스키 사용 -> URLSearchParams가 데이터를 인코딩하기 때문

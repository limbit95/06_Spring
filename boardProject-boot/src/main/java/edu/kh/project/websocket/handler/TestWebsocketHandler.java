 ...
 * Websocket 동작 시 수행할 구문을 작성하는 클래스
 */
@Slf4j
@Component
public class TestWebsocketHandler extends TextWebSocketHandler{
	...
	// SessionHandshakeinterceptor 가 가로챈 연결한 클라이어트의 HttpSession을 갖고 있음
	// (attributes에 추가한 값)
	
	// 동기화된 Set 생성
	private Set<WebSocketSession> sessions = Collections.synchronizedSet(new HashSet<>());
	// -> 여러가지 스레드를 동작하는 환경에서 하나의 컬렉션에 여러 스레드가 접근하고,
	//	  이후 의도치 않은 문제가 발생되지 않게 하기 위해 동기화를 진행하여 
	//	  스레드가 순서대로한 컬렉션에 접근할 수 있도록 변경
	
	// 클라이언트와 연결이 완료되고, 통신할 준비가 되면 실행
	@Override
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		// 연결된 클라이언트의 WebSocketSession 정보를 Set에 추가
		// -> WebSocket에 연결된 클라이언트 정보를 모아둠
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		// WebSocket 연결이 끊긴 클라이언트 정보를 Set에서 제거
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// TextMessage = WebSocket으로 연결된 클라이언트가 전달한 텍스트(내용)가 담겨있는 객체
		
		// message.getPayload() : 통신 시 탑재된 데이터 자체
		log.info("전달 받은 메시지 : {}", message.getPayload());
		
		// 전달 받은 메시지를
		// 현재 해당 WebSocket에 연결된 모든 클라이언트에게 보내기
...
BinaryWebSocketHandler : WebSocketHandler 인터페이스를 상속받아 구현한
